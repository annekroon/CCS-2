...
%Of course, it is a bit of a hassle if we need to make such a list ourselves all the time. But there are files available online that contain stopwords and we can also just load those files and use them!
\end{frame}
...
\end{lstlistingoutput}
	...
	%Let's check again what words are the most used. We see that 'and' is no longer among them. However, there is another strange thing: the two most commonly used words are 'alive-comma', and 'Crying-comma'. 
	
	%Because of these comma's , the stopword 'oh' (that we did include in our stopword list) is not removed, because the computer only sees 'oh-comma'.
...
\begin{lstlistingoutput}
[('alive', 6), ('oh', 4), ('Crying', 2)]
\end{lstlistingoutput}
	
%Let's check the three most common words again. Oh is now the second most used word, because the computer now counts all the 'oh's' (before it considered oh, oh-comma and oh-period as three different words).

%Let's remove the stopwords again so that the word 'oh' (that is now recognized) is removed.
